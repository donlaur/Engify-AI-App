...
const topicsToGenerate = [
  {
    ...
    outline:
      'Write a comprehensive guide on using AI for code review. Include: 1) How to structure code review prompts, 2) Common pitfalls to avoid, 3) Examples of good vs bad prompts, 4) How to handle security concerns, 5) Best practices for different languages.',
  },
  {
    ...
    outline:
      'Create a hands-on tutorial for building a RAG system. Cover: 1) Choosing a vector database, 2) Creating embeddings, 3) Implementing semantic search, 4) Integrating with LLM, 5) Code examples in Python/TypeScript, 6) Common mistakes and how to avoid them.',
  },
  {
    ...
    outline:
      'Detailed guide on reducing AI costs. Include: 1) Token optimization techniques, 2) Caching strategies, 3) Model selection (when to use GPT-4 vs 3.5), 4) Batch processing, 5) Real cost analysis examples, 6) ROI calculations.',
  },
  {
    ...
    outline:
      'Security-focused article on prompt injection. Cover: 1) What is prompt injection, 2) Real-world attack examples, 3) Defense strategies with code, 4) Input validation techniques, 5) Testing for vulnerabilities, 6) Security checklist.',
  },
  {
    ...
    outline:
      'Comparison guide for different AI approaches. Include: 1) Definitions and examples, 2) Cost comparison, 3) Performance trade-offs, 4) Decision framework, 5) Real use cases for each, 6) Implementation examples.',
  },
  {
    ...
    outline:
      'Technical guide on implementing streaming. Cover: 1) Why streaming matters for UX, 2) Server-Sent Events vs WebSockets, 3) Implementation in Next.js, 4) Error handling, 5) Code examples, 6) Performance considerations.',
  },
  {
    ...
    outline:
      'Advanced guide on multi-agent systems. Include: 1) When to use multiple agents, 2) Communication patterns, 3) Task delegation strategies, 4) Example architectures, 5) Code implementation, 6) Common pitfalls.',
  },
  {
    ...
    outline:
      'Practical guide on prompt templates. Cover: 1) What are prompt templates, 2) Variable substitution, 3) Template libraries, 4) Best practices, 5) Examples for common use cases, 6) How to test templates.',
  },
  {
    ...
    outline:
      'Guide on measuring LLM performance. Include: 1) Common metrics (BLEU, ROUGE, perplexity), 2) Custom evaluation criteria, 3) Human evaluation vs automated, 4) A/B testing strategies, 5) Tools and frameworks, 6) Case studies.',
  },
  {
    ...
    outline:
      'Technical guide on context management. Cover: 1) Understanding token limits, 2) Chunking strategies, 3) Sliding window techniques, 4) When to use RAG instead, 5) Code examples, 6) Performance optimization.',
  },
];

async function generateArticle(
  topic: (typeof topicsToGenerate)[0]
): Promise<
  | ((typeof topicsToGenerate)[0] & {
      content: string;
...
Write the article now:`;
  ...
  try {
    const completion = await anthropic.messages.create({
      ...
      temperature: 0.7,
      system:
        'You are an expert technical writer who creates high-quality, actionable content about AI and prompt engineering.',
      messages: [
        {
          role: 'user',
          content: prompt,
...
async function main() {
  ...
  console.log(
    `💰 Estimated cost: $${((totalTokens / 1000) * 0.03).toFixed(2)}\n`
  );
