 ...
 * 
 * Run with: node --loader ts-node/esm scripts/admin/audit-prompt-quality.ts
 */
...
async function main() {
  ...
  try {
    ...
    const incomplete = prompts.filter((p: any) => {
    ...
    if (incomplete.length > 0) {
      ...
      incomplete.slice(0, 10).forEach((p: any, i) => {
        const content = p.content || p.prompt || '';
    ...
    const withScores = prompts.filter((p: any) => p.qualityScore);
    console.log(`\n📊 Quality Scores:`);
    ...
    if (withScores.length > 0) {
      const avgScore = withScores.reduce((sum: number, p: any) => 
    ...
    if (incomplete.length > 0) {
      ...
      console.log(`   Run: pnpm admin:mark-incomplete-inactive\n`);
    }
