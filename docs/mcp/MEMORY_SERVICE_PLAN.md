...
6. **Migration Plan to AWS**
   ...
   - Optional: enable Cloudflare KV sync when domain + account ready.
...
7. **Testing**
   ...
   - Load test: 100 concurrent writes (use Locust or k6) to confirm WAL config.
   - Regression: nightly job to back up SQLite file to S3.

## Open Questions

- Do we need per-tenant encryption-at-rest beyond SQLite WAL? (If yes, evaluate KMS envelope encryption.)
