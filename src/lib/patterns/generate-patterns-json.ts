...
export async function generatePatternsJson(): Promise<void> {
  ...
  const newExportData: PatternsExport = {
    version: '1.0',
  ...
  try {
    ...
    const existingData: PatternsExport = JSON.parse(existingContent);
  ...
  if (shouldWrite) {
    ...
    console.log('[Patterns JSON] Content changed, regenerated file');
  }
}
